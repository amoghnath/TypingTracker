...
const WPMTextArea = ({ mainText }) => {
	...
	useKey((key) => {
		if(key !== "Backspace"){
			...
			setTypingState((prevTypingState) => {
				return {
					...prevTypingState,
					charsTyped: prevTypingState.charsTyped.filter((_,i) => i !== prevTypingState.charsTyped.length - 1)
				}
			})
		}
	})
	...
	useEffect(() => {
		const characters = mainTextLines.current.map((line) => lineToChars(line)).flat();
	...
	const accurateText = `${correct} / ${typingState.charsTyped.length} (${accuracy.toFixed(0)}%)`
